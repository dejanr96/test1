...
async def run_backtest():
    print("--- STARTING 3-MONTH BACKTEST (Long Term) ---")
    
    # 90 days * 24h * 60m * 60s * 1000ms
    ninety_days_ms = 90 * 24 * 60 * 60 * 1000
    now_ms = int(time.time() * 1000)
    since_ms = now_ms - ninety_days_ms
    
    # Approx 130,000 minutes in 90 days
    LIMIT = 130000
    
    print(f"Fetching {LIMIT} candles starting from 90 days ago (Timestamp: {since_ms})...")
    ...
    df = await data_service.fetch_ohlcv(limit=LIMIT, since=since_ms)
    ...
    try:
        ...
        while True:
            ...
            if step_count % 10000 == 0:
                print(f"Step {step_count}/{len(df)}...")
            ...
            if dones[0]:
                ...
                print("\n" + "="*40)
                print(f"FINAL 3-MONTH BACKTEST RESULTS")
